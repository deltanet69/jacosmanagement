      ...
      <main className="relative px-12 pb-12">
        <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gold to-white">Sedang Dipanggil</h2>
          </div>
        ...
        {queue.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-white/20 rounded-4xl bg-white/5 mb-14">
            <p className="text-white/40 text-2xl font-display font-bold">Belum ada antrian penjemputan.</p>
              ...
              <div className="bg-gradient-to-br from-gold/20 to-sky/20 border-2 border-gold/50 rounded-[3rem] p-10 relative overflow-hidden shadow-[0_0_50px_rgba(255,215,0,0.15)] animate-pulse-slow pop-in">
                ...
                <div className="relative z-10 flex items-start gap-8">
                  ...
                  <div className="flex-1">
                    <p className="text-gold uppercase tracking-widest font-bold text-sm mb-2">Mohon Menuju Loket</p>
                    ...
                    <div className="inline-flex items-center gap-3 bg-black/40 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10">
                      ...
                      <span className="text-white font-bold text-xl">{queue[0].picked_by_name}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {queue.length > 1 && (
              <div className="xl:w-1/2">
                <h3 className="text-white/40 font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                  Antrian Selanjutnya <span className="bg-sky/20 text-sky px-2 py-0.5 rounded-md text-xs">{queue.length - 1}</span>
                </h3>
                <div className="grid sm:grid-cols-2 gap-4">
                  {queue.slice(1, 7).map((q, idx) => (
                    <div key={q.id} className="bg-white/5 border border-white/10 rounded-3xl p-5 slide-in-right" style={{ animationDelay: `${idx * 0.1}s` }}>
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-display text-lg font-bold leading-tight">{q.students?.full_name?.split(' ')[0]}</p>
                            <p className="text-white/40 text-xs mt-0.5">Kelas {q.students?.class_id || '-'}</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
                        <span className="text-white/40 text-[10px] uppercase font-bold tracking-wider">Estimasi</span>
        ...
        <div className="border-t-2 border-white/10 pt-8 mt-auto">
          <p className="text-white/40 text-sm font-bold uppercase tracking-widest mb-6">Baru Saja Dijemput (Selesai)</p>
          <div className="flex flex-wrap gap-4">
            {completed.length === 0 ? (
              <span className="text-white/20 text-sm font-medium">—</span>
            ) : (
              completed.map((c) => (
                <div key={c.id} className="bg-leaf/10 border border-leaf/20 px-5 py-2.5 rounded-2xl flex items-center gap-3 slide-in-right">
                  <span className="w-6 h-6 rounded-full bg-leaf text-white flex items-center justify-center text-xs font-bold">✓</span>
                  <span className="text-white font-bold">{c.students?.full_name?.split(' ')[0]}</span>
                  <span className="text-white/40 text-xs">({c.students?.class_id})</span>
                </div>
              ))
            )}
          </div>
