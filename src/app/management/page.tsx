...
export default function DashboardOverview() {
  ...
  if (isLoading && !data) {
    return (
      <div className="py-20 text-center text-ink-300 font-medium">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-5">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          <button
            ...
            className="h-10 sm:h-11 px-3.5 rounded-xl sm:rounded-2xl bg-white border border-ink/10 text-ink-400 hover:text-sky text-xs font-bold flex items-center gap-1.5 shadow-2xs transition"
          >
            <RefreshCw size={13} className={isRefreshing ? "animate-spin text-sky" : ""} /> Refresh
          </button>

          <Link href="/management/informasi/tambah">
            <Button className="h-10 sm:h-11 px-4 sm:px-5 rounded-xl sm:rounded-2xl bg-ink hover:bg-ink/90 text-white font-bold text-xs shadow-md">
          ...
          <Link href="/penjemputan-app" target="_blank">
            <Button
              variant="outline"
              className="h-10 sm:h-11 px-3.5 sm:px-4 rounded-xl sm:rounded-2xl bg-white border-purple-200 text-purple-700 hover:bg-purple-50 font-bold text-xs shadow-2xs"
            >
                ...
                {d.admissions.recentApplicants.map((applicant) => {
                  ...
                  return (
                    <Link
                      ...
                      className="p-3.5 sm:p-4 rounded-2xl bg-cloud/40 hover:bg-cloud/90 border border-ink/5 hover:border-sky/20 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 group"
                    >
        ...
        <div className="space-y-5 sm:space-y-6">
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 border border-ink/5 shadow-sm space-y-4">
            ...
            <Link href="/management/absensi" className="block pt-1">
              <Button
                variant="outline"
                className="w-full h-10 rounded-2xl border-ink/15 font-bold text-xs text-ink-400 hover:text-ink hover:bg-cloud"
              >
              ...
              <div className="space-y-2.5">
                {d.announcements.map((ann) => (
                  <Link
                    ...
                    className="block p-3 rounded-2xl bg-cloud/40 hover:bg-sky-50/50 border border-ink/5 transition text-xs space-y-1"
                  >
                    <div className="flex items-center justify-between">
