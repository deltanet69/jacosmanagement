...
export default function ApplicantDetailClient({
  ...
  const [data, setData] = useState<any>(initialApplicant);
  const router = useRouter();

  // Approval Modal States (Batch Selection)
  const [showApprovalModal, setShowApprovalModal] = useState(false);
  const [selectedBatch, setSelectedBatch] = useState<AdmissionBatchKey>(
    initialApplicant?.batch && ["BATCH_1", "BATCH_2", "BATCH_3"].includes(initialApplicant.batch)
      ? initialApplicant.batch
      : getCurrentActiveBatch()
  );
  ...
  // Origin URL (fix hydration mismatch)
  const [siteOrigin, setSiteOrigin] = useState("https://jacosmanagement.vercel.app");
  useEffect(() => {
  ...
  const guardians: any[] = Array.isArray(rawGuardians)
    ? rawGuardians
    : rawGuardians
    ? [rawGuardians]
    : [];
  ...
  const primaryGuardian =
    guardians.find((g: any) => g?.email && typeof g.email === "string" && g.email.includes("@")) ||
    guardians[0];
  const targetParentEmail = primaryGuardian?.email || null;

  const relationLabel = (rel?: string) => {
    if (!rel) return "Orang Tua / Wali";
    const upper = rel.toUpperCase().trim();
    if (upper === "FATHER" || upper === "AYAH") return "Ayah / Daddy";
    if (upper === "MOTHER" || upper === "IBU") return "Ibu / Mommy";
    if (upper === "GUARDIAN" || upper === "WALI") return "Wali Murid";
    return rel;
  };
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="font-mono text-sm text-ink truncate bg-cloud rounded-2xl px-4 py-2.5 mt-2 border border-ink/5">
                {siteOrigin}/reg/{data.registration_token}
              </p>
            </div>
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
              {guardians.length > 0 ? (
                guardians.map((g: any, idx: number) => {
                  const relUpper = (g.relation || "").toUpperCase().trim();
                  const isFather = relUpper === "FATHER" || relUpper === "AYAH";
                  const isMother = relUpper === "MOTHER" || relUpper === "IBU";
                  ...
                  const rawPhone = (g.phone || "").replace(/[^0-9]/g, "");
                  const waPhone = rawPhone.startsWith("0") ? `62${rawPhone.slice(1)}` : rawPhone;

                  return (
                    <div
                      key={g.id || `${g.relation}-${idx}`}
                      className={`p-5 rounded-3xl border ${theme.bg} shadow-xs space-y-4 text-sm relative transition-all hover:shadow-md`}
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        {g.nik && g.nik !== "-" && (
                          <div>
                            <p className="text-[11px] font-bold text-ink-300 uppercase tracking-wider mb-0.5">NIK (Kependudukan)</p>
                            <p className="font-mono font-bold text-ink text-xs bg-white/80 px-2.5 py-1 rounded-lg border border-ink/5 inline-block">
                              {g.nik}
                            </p>
                          </div>
                        )}
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-ink text-xs">{g.occupation || "-"}</p>
                        </div>

                        {g.monthly_income && g.monthly_income !== "-" && (
                          <div>
                            <p className="text-[11px] font-bold text-ink-300 uppercase tracking-wider mb-0.5">Penghasilan Bulanan</p>
                            <p className="font-semibold text-sky-800 bg-white/90 px-3 py-1 rounded-xl text-xs border border-sky-200/80 inline-block shadow-2xs">
                              {g.monthly_income}
                            </p>
                          </div>
                        )}

                        {g.education_level && g.education_level !== "-" && g.education_level !== "S1" && (
                          <div>
                            <p className="text-[11px] font-bold text-ink-300 uppercase tracking-wider mb-0.5">Pendidikan Terakhir</p>
                            <p className="font-medium text-ink text-xs">{g.education_level}</p>
                          </div>
                        )}

                        {g.address && g.address !== "-" && (
                          <div>
                            <p className="text-[11px] font-bold text-ink-300 uppercase tracking-wider mb-0.5">Alamat Domisili</p>
                            <p className="font-medium text-ink text-xs leading-relaxed">{g.address}</p>
                          </div>
                        )}
                      </div>
                    </div>
