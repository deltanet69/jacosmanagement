    ...
    <div className="space-y-6 sm:space-y-7 pb-16 w-full">
      ...
      {showScannerModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-5 sm:p-7 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-ink/5 space-y-5 relative">
            <button
              onClick={() => setShowScannerModal(false)}
              className="absolute top-5 right-5 w-8 h-8 rounded-full bg-cloud flex items-center justify-center text-ink-400 hover:text-ink hover:bg-ink/10 transition"
            >
              <X size={16} />
            </button>
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-1.5">
                <div className="w-9 h-9 rounded-2xl bg-sky-50 text-sky flex items-center justify-center shrink-0">
                  <QrCode size={20} />
                </div>
                <div>
                  <h3 className="font-display text-lg sm:text-xl font-bold text-ink">Scanner Penjemputan</h3>
                  <p className="text-xs text-ink-400">Verifikasi QR Penjemputan Siswa</p>
                </div>
              </div>

              {/* Mode Switcher */}
              <div className="grid grid-cols-2 bg-cloud rounded-2xl p-1 mt-3.5 border border-ink/5">
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`py-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                    scannerMode === "HARDWARE" ? "bg-white text-ink shadow-2xs" : "text-ink-400 hover:text-ink"
                  }`}
                >
                  <Smartphone size={13} /> Scan QR Barcode
                </button>
                <button
                  type="button"
                  onClick={() => setScannerMode("MANUAL")}
                  className={`py-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                    scannerMode === "MANUAL" ? "bg-white text-ink shadow-2xs" : "text-ink-400 hover:text-ink"
                  }`}
                >
            ...
            {scannerMode === "HARDWARE" && (
              <div className="space-y-3.5">
                <div className="bg-ink rounded-2xl p-6 flex flex-col items-center justify-center text-center relative overflow-hidden aspect-video">
                  <div className="absolute inset-0 bg-gradient-to-br from-sky/20 to-purple-600/20 opacity-30"></div>
                  <div className="absolute inset-3 border-2 border-dashed border-white/20 rounded-xl"></div>

                  <div className="w-11 h-11 rounded-2xl bg-white/10 text-white flex items-center justify-center mb-2.5 relative z-10 animate-bounce">
                    <QrCode size={24} />
                  </div>
                  <p className="text-white font-bold text-xs sm:text-sm relative z-10">Siap Menerima Scan QR</p>
                  <p className="text-white/50 text-[11px] mt-0.5 max-w-xs relative z-10">
                ...
                >
                  <Input
                    placeholder="Atau ketik ID Siswa / Kode QR manual..."
                    value={qrInput}
                    onChange={(e) => setQrInput(e.target.value)}
                    className="h-10 rounded-xl text-xs bg-cloud border-ink/10"
                  />
                  <Button type="submit" className="h-10 px-4 rounded-xl bg-sky hover:bg-sky-600 text-white font-bold text-xs">
                    Kirim
                  </Button>
                </form>
                ...
                {recentScanResult && (
                  ...
                  >
                    <p className="font-bold flex items-center gap-1.5">
                      {recentScanResult.success ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />}
            ...
            {scannerMode === "MANUAL" && (
              <form onSubmit={handleManualSubmit} className="space-y-3.5">
                <div>
                  <Label className="block text-xs font-bold mb-1.5 text-ink-400 uppercase tracking-wider">
                    Cari Siswa (Nama / NIS)
                  </Label>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-10 pl-9 rounded-xl bg-cloud border-ink/10 text-xs"
                    />
                  </div>

                  {/* Results list */}
                  {searchResults.length > 0 && !selectedStudent && (
                    <div className="mt-2 max-h-36 overflow-y-auto rounded-xl border border-ink/10 bg-white shadow-md divide-y divide-ink/5">
                      {searchResults.map((s) => (
                        <div
                          key={s.id}
                          onClick={() => {
                            setSelectedStudent(s);
                            setManualPickerName(s.authorized_pickup_name || "");
                            setManualQuery("");
                            setSearchResults([]);
                          }}
                        ...
                        >
                          ...
                          <span className="text-[11px] font-bold text-sky">Pilih</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {selectedStudent && (
                    <div className="mt-2.5 p-3 rounded-xl bg-sky-50/70 border border-sky-200 flex items-center justify-between">
                      <div>
                        <span className="text-[10px] font-bold text-sky-700 uppercase tracking-wider">Siswa Terpilih</span>
                        <p className="font-bold text-ink text-xs sm:text-sm">{selectedStudent.full_name}</p>
                        <p className="text-[11px] text-ink-400">
                          {selectedStudent.className} • Penjemput: {selectedStudent.authorized_pickup_name || "Orang Tua"}
                        </p>
                      </div>
                      <Button
                        type="button"
                        ...
                        className="text-xs text-coral hover:bg-coral-50 rounded-lg h-7 px-2"
                      >
                        Ganti
                      </Button>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <div>
                    <Label className="block text-xs font-bold mb-1 text-ink-400 uppercase tracking-wider">
                      Nama Penjemput
                    </Label>
                    <Input
                      value={manualPickerName}
                      onChange={(e) => setManualPickerName(e.target.value)}
                      placeholder="Contoh: Bpk. Ahmad"
                      className="h-10 rounded-xl bg-cloud border-ink/10 text-xs"
                      required
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={manualPickerRelation}
                      onChange={(e) => setManualPickerRelation(e.target.value)}
                      placeholder="Contoh: Ayah / Supir"
                      className="h-10 rounded-xl bg-cloud border-ink/10 text-xs"
                      required
                    />
                  </div>
                ...
                <Button
                  type="submit"
                  disabled={!selectedStudent || isSubmittingManual}
                  className="w-full h-11 bg-sky hover:bg-sky-600 text-white font-bold rounded-xl text-xs sm:text-sm shadow-md"
                >
                  {isSubmittingManual ? "Memproses..." : "Masuk ke Antrian Penjemputan"}
                </Button>
              </form>
            )}
          </div>
